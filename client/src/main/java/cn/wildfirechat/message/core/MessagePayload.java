...
public class MessagePayload implements Parcelable {
    ...
    public MessagePayload(ProtoMessageContent protoMessageContent) {
        ...
        //fixme 暂时硬编码解决下红包类型的错误
        if (TextUtils.isEmpty(protoMessageContent.getExtra()) &&
                (protoMessageContent.getType() == MessageContentType.ContentType_Lucky_Money ||
                        protoMessageContent.getType() == MessageContentType.ContentType_Lucky_Money_AGroup ||
                        protoMessageContent.getType() == MessageContentType.ContentType_Transfer_Money ||
                        protoMessageContent.getType() == MessageContentType.ContentType_Lucky_Money_Accept_Single ||
                        protoMessageContent.getType() == MessageContentType.ContentType_Lucky_Money_Accept_AGroup ||
                        protoMessageContent.getType() == MessageContentType.ContentType_Transfer_Money_Accept )) {
            extra = protoMessageContent.getContent();
        }
    }
