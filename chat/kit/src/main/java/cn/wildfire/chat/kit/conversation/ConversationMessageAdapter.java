...
public class ConversationMessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ConversationMessageAdapter(ConversationFragment fragment) {
        ...
        sp = fragment.getContext().getSharedPreferences("config", Context.MODE_PRIVATE);
        mute = sp.getString("mute", "no");
    }
    ...
    public void addNewMessage(UiMessage message) {
        ...
        if (mute.equals("no")) {
            ...
            // 播放收到红包的声音
            if (message.message.content.getType() == MessageContentType.ContentType_Lucky_Money_AGroup) {
                mediaPlayer = MediaPlayer.create(fragment.getContext(), R.raw.message_bonus);

            } else {
                // 播放普通消息的声音
                mediaPlayer = MediaPlayer.create(fragment.getContext(), R.raw.message_normal);
            }
            mediaPlayer.start();
        }
