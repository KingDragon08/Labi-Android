...
public class MessageViewModel extends ViewModel implements OnReceiveMessageListener,
    ...
    public void onReceiveMessage(List<Message> messages, boolean hasMore) {
        if (messageLiveData != null && messages != null) {
            for (Message msg : messages) {
                ...
                //如果是收红包消息，需要缓存到本地
                if (msg.content.getClass().isAssignableFrom(LuckyMoneyReceiveMessageContent.class)) {
                    RedPacketDetail redPacketDetail = new Gson().fromJson(msg.content.extra, RedPacketDetail.class);
                    redPacketDetail.messageUid = msg.messageUid;
                    RedPacketRepository.getInstance().insertRedPacket(redPacketDetail);
                    //同时触发一下消息更新
                    Message originMessage = ChatManager.Instance().getMessageByUid(redPacketDetail.messageUid);
                    postNewMessage(new UiMessage(originMessage));
                }
            }
        }
    }
    ...
    public void onSendSuccess(Message message) {
        ...
        //如果是收红包消息，需要缓存到本地
        if (LuckyMoneyReceiveMessageContent.class.isAssignableFrom(message.content.getClass())) {
            RedPacketDetail redPacketDetail = new Gson().fromJson(message.content.extra, RedPacketDetail.class);
            long rs = RedPacketRepository.getInstance().insertRedPacket(redPacketDetail);
            Log.v("TAG","insert rs = " + rs);
            //同时触发一下消息更新
            Message originMessage = ChatManager.Instance().getMessageByUid(redPacketDetail.messageUid);
            postNewMessage(new UiMessage(originMessage));
        }
